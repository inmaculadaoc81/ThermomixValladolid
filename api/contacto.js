...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta ThermomixRepair - thermomixrepair.es | ${nombre}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1B2420"><h2 style="color:#1E6E4F">Nueva consulta desde ThermomixRepair</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Thermomix:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#5C6B63">Enviado desde el formulario de thermomixrepair.es</p></div>`;
    const mime=[`From: ThermomixRepair <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
